...
void cellmaker::on_pushButton_8_clicked() {
    ...
    ManualText english = {
        ...
        "<ul>"
        "<li><b>Materials:</b> Assign materials to each region. The software automatically handles material IDs for the PHITS <code>[ Material ]</code> section.</li>"
        "<li><b>Source Configuration:</b> Define the particle beam as either a <b>Point</b> source or a broad <b>Parallel</b> beam. The <code>maxcas</code> and <code>maxbch</code> parameters control statistical history generation.</li>"
    ...
    ManualText japanese = {
        ...
        "<ul>"
        "<li><b>材料 (Materials):</b> 各領域に材料を割り当てます。ソフトウェアはPHITSの <code>[ Material ]</code> セクション用の材料識別番号（Material ID）を自動的に処理します。</li>"
        "<li><b>線源設定 (Source Configuration):</b> 粒子ビームを<b>点線源 (Point)</b> または広範な<b>平行ビーム (Parallel)</b> として定義します。統計ヒストリーの生成は <code>maxcas</code> と <code>maxbch</code> パラメータで制御されます。</li>"
        "<li><b>タリー / 出力 (Tallies):</b> チェックボックスを使用して、形状確認 (<code>[ T - Gshow ]</code>)、飛跡の可視化 (<code>[ T - Track ]</code>)、および個別の細胞コンパートメント内の高精度な微視的付与線量計算 (<code>[ T - Deposit ]</code>) のカードを自動生成します。</li>"
        "</ul>"
